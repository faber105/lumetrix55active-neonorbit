...
def _features(x: dict) -> List[float]:
    ...
    high20 = float(np.max(hi[-21:-1]))
    low20 = float(np.min(lo[-21:-1]))
    range20 = max(high20 - low20, 1e-9)
    body = float(close[i] - op[i])
    upper_wick = float(hi[i] - max(op[i], close[i]))
    lower_wick = float(min(op[i], close[i]) - lo[i])
    slope = float(x["ema20"][i] - x["ema20"][-6]) / atr
    return [
        _clip((float(x["rsi"][i]) - 50.0) / 10.0),
        _clip((float(close[i]) - float(x["ema20"][i])) / atr),
        _clip((float(x["ema20"][i]) - float(x["ema50"][i])) / atr),
        _clip((float(x["ema50"][i]) - float(x["ema200"][i])) / atr),
        _clip(float(x["macd_hist"][i]) / atr),
        _clip((float(close[i]) - mid) / sd),
        _clip((atr / max(abs(float(close[i])), 1e-9)) * 1000.0),
        _clip((float(close[i]) - low20) / range20 * 2.0 - 1.0),
        _clip(body / atr),
        _clip(upper_wick / atr),
        _clip(lower_wick / atr),
        _clip(slope),
    ]
...
def ema_trend(candles: list) -> Optional[StrategyCandidate]:
    ...
    atr = max(float(x["atr"][i]), 1e-9)
    e20, e50, e200 = float(x["ema20"][i]), float(x["ema50"][i]), float(x["ema200"][i])
    rsi = float(x["rsi"][i])
    hist = float(x["macd_hist"][i])
    prev_hist = float(x["macd_hist"][i - 1])
    near_ema20 = abs(price - e20) <= 0.65 * atr
    ...
    separation = min(abs(e20 - e50) / atr, 2.0)
    slope = abs(e20 - float(x["ema20"][-6])) / atr
    momentum = min(abs(hist) / atr * 15.0, 1.5)
    confidence = min(70.0 + 6.0 * min(separation, 1.5) + 4.0 * min(slope, 1.5) + 3.0 * momentum, 91.0)
...
def bollinger_reversal(candles: list) -> Optional[StrategyCandidate]:
    x = _base(candles); i=-1
    price=float(x["close"][i]); prev_close=float(x["close"][i-1]); upper=float(x["bb_upper"][i]); lower=float(x["bb_lower"][i]); prev_upper=float(x["bb_upper"][i-1]); prev_lower=float(x["bb_lower"][i-1])
    if any(math.isnan(v) for v in [upper,lower,prev_upper,prev_lower]): return None
    atr=max(float(x["atr"][i]),1e-9); rsi=float(x["rsi"][i]); ema_gap=abs(float(x["ema20"][i])-float(x["ema50"][i]))/atr
    ranging=ema_gap<=1.25; bull=ranging and prev_close<=prev_lower and price>lower and rsi<=38; bear=ranging and prev_close>=prev_upper and price<upper and rsi>=62
    ...
    confidence=min(71.0+min(excursion*8.0,8.0)+min(abs(rsi-50.0)/4.0,8.0)+max(0.0,(1.25-ema_gap)*3.0),92.0)
...
def atr_breakout(candles: list) -> Optional[StrategyCandidate]:
    x=_base(candles); i=-1
    price=float(x["close"][i]); prev_price=float(x["close"][i-1]); atr=max(float(x["atr"][i]),1e-9); atr_avg=float(np.mean(x["atr"][-21:-1])); high20=float(np.max(x["high"][-21:-1])); low20=float(np.min(x["low"][-21:-1])); hist=float(x["macd_hist"][i]); rsi=float(x["rsi"][i]); expansion=atr/max(atr_avg,1e-9)
    ...
    confidence=min(72.0+min((expansion-1.0)*18.0,9.0)+min(break_size*9.0,7.0)+min(abs(hist)/atr*12.0,5.0),93.0)
