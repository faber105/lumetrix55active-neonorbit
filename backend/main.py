...
logging.basicConfig(level=logging.INFO); logger=logging.getLogger('alphapulse')
@asynccontextmanager
...
app.include_router(auth.router,prefix='/api/auth',tags=['auth']); app.include_router(signals.router,prefix='/api/signals',tags=['signals']); app.include_router(market.router,prefix='/api/market',tags=['market']); app.include_router(stats.router,prefix='/api/stats',tags=['stats']); app.include_router(settings.router,prefix='/api/settings',tags=['settings']); app.include_router(websocket.router,prefix='/ws',tags=['websocket'])
@app.get('/health')
async def health(): return {'status':'ok','service':'alphapulsesbot','market':await market_data.health()}
...
# Mount the original alphapulsesbot Mini App last, so API/webhook routes keep priority.
miniapp_dist = Path(__file__).resolve().parents[1] / 'miniapp' / 'dist'
