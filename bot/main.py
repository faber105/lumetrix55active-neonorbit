...
class AntiSpam(BaseMiddleware):
    async def __call__(self,handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]],event: TelegramObject,data: Dict[str, Any]) -> Any:
        ...
        if user and is_rate_limited(user.id): return None
        return await handler(event, data)


bot = Bot(token=BOT_TOKEN, default=DefaultBotProperties(parse_mode="HTML"))
dp = Dispatcher(); dp.message.middleware(AntiSpam()); dp.callback_query.middleware(AntiSpam())
...
async def api(method: str, path: str, **kwargs):
    try:
        ...
        logger.warning("API %s %s failed: %s", method, path, exc); return None
...
async def set_status(telegram_id: int, status: str) -> bool:
    data=await api("POST","/api/auth/status",json={"telegram_id":telegram_id,"status":status,"secret":ADMIN_SECRET}); return bool(data and data.get("ok"))
async def verified_or_reply(message: types.Message) -> bool:
    user=await get_user(message.from_user.id)
    if not user or user.get("status") != "VERIFIED": await message.answer("Доступ ограничен. Напишите /start для регистрации."); return False
    return True


def kb_register(telegram_id: int):
    builder=InlineKeyboardBuilder(); builder.row(InlineKeyboardButton(text="Зарегистрироваться у брокера",url=f"{BACKEND_URL}/api/auth/go?uid={telegram_id}")); builder.row(InlineKeyboardButton(text="✅ Я зарегистрировался",callback_data="verify:i_registered")); builder.row(InlineKeyboardButton(text="Написать менеджеру",callback_data="support")); return builder.as_markup()
def kb_main():
...
async def cmd_start(message: types.Message):
    telegram_id=message.from_user.id; user=await create_user(telegram_id,message.from_user.username or "",message.from_user.full_name or "")
    if not user: await message.answer("Сервер недоступен. Попробуйте позже."); return
    if telegram_id==ADMIN_ID and user.get("status")!="VERIFIED": await set_status(telegram_id,"VERIFIED"); user=await get_user(telegram_id) or user
    status=user.get("status","NEW")
    if status=="BLOCKED": await message.answer("Доступ заблокирован. Обратитесь к менеджеру "+MANAGER); return
    if status=="PENDING": await message.answer(f"⏳ Ваша заявка на проверке.\nВаш ID: <code>{telegram_id}</code>"); return
    if status=="VERIFIED":
        await message.answer("⚡ <b>Добро пожаловать в AlphaPulse!</b>\n\n🔥 <b>VIP Сигналы</b> — автоматические OTC-сетапы\n📊 <b>Market AI</b> — анализ OTC-пар\n📈 <b>3 стратегии</b> — EMA Trend, Bollinger Reversal, ATR Breakout\n🤖 <b>Online ML</b> — обучается на закрытых сигналах\n\nСигналы вероятностные и не гарантируют прибыль.",reply_markup=kb_main()); await message.answer("Открыть AlphaPulse 👇",reply_markup=kb_miniapp()); return
...
async def request_verification(callback: types.CallbackQuery):
    ...
    if user.get("click_time"):
        clicked=datetime.fromisoformat(user["click_time"].replace("Z","+00:00")); clicked=clicked.replace(tzinfo=timezone.utc) if clicked.tzinfo is None else clicked; remaining=MIN_CLICK-(datetime.now(timezone.utc)-clicked).total_seconds()
    ...
    try: await bot.send_message(ADMIN_ID,"🔔 <b>Новая заявка</b>\n"f"ID: <code>{telegram_id}</code>\n"f"@{callback.from_user.username or '—'}\n"f"/verify {telegram_id} | /block {telegram_id}")
    except Exception: pass

@dp.callback_query(F.data == "support")
async def support_callback(callback: types.CallbackQuery): await callback.answer(); await callback.message.answer(f"Поддержка: {MANAGER}\nID: <code>{callback.from_user.id}</code>")
...
async def admin_pending(message: types.Message):
    if message.from_user.id!=ADMIN_ID: return
    users=await api("GET",f"/api/auth/users?secret={ADMIN_SECRET}") or []; pending_users=[user for user in users if user["status"]=="PENDING"]
...
async def admin_users(message: types.Message):
    if message.from_user.id!=ADMIN_ID: return
    users=await api("GET",f"/api/auth/users?secret={ADMIN_SECRET}") or []; lines=["Все пользователи:",""]; lines.extend(f"{user['status']} | {user['telegram_id']} @{user.get('username') or '—'}" for user in users[:30]); await message.answer("\n".join(lines))

@dp.message(Command("stats"))
async def admin_stats(message: types.Message):
    if message.from_user.id!=ADMIN_ID: return
    users=await api("GET",f"/api/auth/users?secret={ADMIN_SECRET}") or []; await message.answer("Статистика:\n"f"Всего: {len(users)}\n"f"Верифицированы: {sum(u['status']=='VERIFIED' for u in users)}\n"f"На проверке: {sum(u['status']=='PENDING' for u in users)}\n"f"Заблокированы: {sum(u['status']=='BLOCKED' for u in users)}")
...
async def vip_signals(message: types.Message):
    ...
    lines=["VIP Сигналы",""]; lines.extend(f"{signal['pair']} {signal['timeframe']} | {'CALL' if signal['direction']=='BUY' else 'PUT'} | {signal['confidence']}%" for signal in signals); await message.answer("\n".join(lines),reply_markup=kb_miniapp())
